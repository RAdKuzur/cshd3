        ...
        <form @submit.prevent="handleSubmit">
          ...
          <div class="mb-8">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div v-if="(formData.type === 1 || formData.type === 4 || formData.type === 5 || formData.type === 6 ) || formData.type === 2">
                <label class="block text-sm font-medium text-gray-700 mb-2">
                  Кто (принимает) *
                </label>
                <select disabled
                    v-model.number="formData.to"
                    :required="(formData.type === 1 || formData.type === 4 || formData.type === 5 || formData.type === 6) || formData.type === 2"
                    class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition-colors"
                >
                  <option value="">Выберите сотрудника</option>
                  <option
          ...
          <div class="flex items-center justify-end gap-4 pt-6 border-t border-gray-200">
            ...
            <button
                type="submit"
                :disabled="isSubmitting || isLoading || !formData.type || !formData.date || ((formData.type === 1 || formData.type === 4 || formData.type === 5 || formData.type === 6) && !formData.to) || (formData.type === 2 && (!formData.from || !formData.to)) || (formData.type === 3 && !formData.from)"
