    ...
    <div class="max-w-8xl mx-auto">
        ...
        <div class="flex justify-between items-center">
          <div class="flex items-center gap-4">
            ...
            <div>
              ...
              <p class="text-gray-600 mt-2">Всего материалов: {{ filteredItems.length }}</p>
            </div>
          </div>
          <router-link to="/things/resources/create">
        ...
        <div class="mt-6 flex gap-4 flex-wrap">
          ...
          <div class="flex gap-4 flex-wrap">
            ...
            >
              ...
              <option v-for="(label, id) in resourceTypes" :key="id" :value="parseInt(id)">
                {{ label }}
              </option>
            </select>
      ...
      <div v-else class="bg-white rounded-2xl shadow-lg border border-gray-200 overflow-hidden">
          ...
          <table class="w-full">
            ...
            >
              ...
              <td class="px-6 py-4">
                <div class="flex items-center">
                  <div class="flex-shrink-0 h-10 w-10 bg-gradient-to-br from-blue-100 to-indigo-100 rounded-lg flex items-center justify-center shadow-sm">
                    <span class="text-sm font-bold text-indigo-700">#{{ item.id }}</span>
                  </div>
                  <div class="ml-4">
              ...
              <td class="px-6 py-4 whitespace-nowrap">
                <div class="flex items-center">
                  <div :class="getTypeColor(item.type)" class="flex-shrink-0 h-8 w-8 rounded-lg flex items-center justify-center mr-3 shadow-sm">
              ...
              <td class="px-6 py-4 whitespace-nowrap">
                <div class="flex items-center">
                  ...
                  <div>
                    <div class="text-xl font-bold text-gray-900" :class="getAmountColorClass(item.amount)">
                      {{ item.amount }}
                    </div>
                    <div class="text-xs text-gray-500">
              ...
              <!-- Статус запасов -->
              <td class="px-6 py-4 whitespace-nowrap">
                  <span :class="getStockStatusClass(item.amount)" class="px-3 py-1 text-xs font-semibold rounded-full">
                    {{ getStockStatusLabel(item.amount) }}
                  </span>
              </td>
              ...
              <td class="px-6 py-4 whitespace-nowrap">
                <div class="flex items-center space-x-2">
                  <!-- Кнопка просмотра -->
                  <router-link :to="`/things/resources/view/${item.id}`">
                  ...
                  <router-link :to="`/things/resources/edit/${item.id}`">
        ...
        <div v-if="!isLoading && filteredItems.length === 0" class="text-center py-12">
          ...
          <h3 class="mt-2 text-sm font-medium text-gray-900">Материалы не найдены</h3>
          <p class="mt-1 text-sm text-gray-500">
            {{ searchQuery ? 'Попробуйте изменить параметры поиска' : 'Добавьте первый материал' }}
          </p>
        </div>
...
// Реактивные данные
const headers = ref([
  { key: 'name', label: 'Название материала' },
  { key: 'type', label: 'Тип материала' },
  { key: 'amount', label: 'Количество' },
  { key: 'stock_status', label: 'Статус запасов' }
])
...
const loadData = async () => {
  try {
    ...
    if (typesResponse.data.success) {
      resourceTypes.value = typesResponse.data.data || {}
    }

    if (resourcesResponse.data.success) {
      resources.value = resourcesResponse.data.data || []
    } else {
      throw new Error('Не удалось загрузить список материалов')
    }
...
const filteredItems = computed(() => {
  ...
  if (searchQuery.value) {
    ...
    filtered = filtered.filter(item =>
        ...
        (getResourceTypeLabel(item.type) && getResourceTypeLabel(item.type).toLowerCase().includes(query))
    )
  }
  ...
  filtered = [...filtered].sort((a, b) => {
    ...
    if (sortKey.value === 'name') {
      ...
      aVal = getResourceTypeLabel(a.type) || ''
      bVal = getResourceTypeLabel(b.type) || ''
...
const confirmDelete = async (item) => {
  if (confirm(`Вы уверены, что хотите удалить материал "${item.name}"?`)) {
    try {
      await axios.delete(`${BACKEND_URL}/api/resources/${item.id}`)

      // Удаляем из локального списка
      const index = resources.value.findIndex(i => i.id === item.id)
      if (index !== -1) {
