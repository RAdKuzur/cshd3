<template>
  <div class="p-6 bg-gradient-to-br from-gray-50 to-blue-50 min-h-screen">
    <div class="max-w-4xl mx-auto">
      ...
      <div class="mb-8">
        <div class="flex items-center justify-between">
          <div class="flex items-center gap-4">
            <router-link
                :to="`/things/resources/view/${resourceId}`"
            ...
            <div>
              <h1 class="text-3xl font-bold text-gray-900">Редактирование материала</h1>
              <p class="text-gray-600 mt-2">Материал</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <div v-if="!isLoading && !error" class="bg-white shadow-lg border border-gray-200 rounded-xl">
        <div class="p-6">
          ...
          <form @submit.prevent="handleSubmit">
            ...
            <!-- Сравнение изменений -->
            <div v-if="hasChanges" class="mb-8 bg-yellow-50 border border-yellow-200 rounded-lg p-4">
              <h3 class="text-sm font-medium text-yellow-800 mb-3">Изменения</h3>
              <div class="space-y-3">
                <div v-if="form.name !== originalData.name" class="flex items-center justify-between">
                  <span class="text-sm text-yellow-700">Название:</span>
                  <div class="text-right">
                    <div class="text-xs line-through text-gray-500">{{ originalData.name }}</div>
                    <div class="font-medium text-yellow-900">{{ form.name }}</div>
                  </div>
                </div>
                <div v-if="form.type !== originalData.type" class="flex items-center justify-between">
                  <span class="text-sm text-yellow-700">Тип:</span>
                  <div class="text-right">
                    <div class="text-xs line-through text-gray-500">{{ getResourceTypeLabel(originalData.type) }}</div>
                    <div class="font-medium text-yellow-900">{{ getResourceTypeLabel(form.type) }}</div>
                  </div>
                </div>
                <div v-if="parseInt(form.amount) !== originalData.amount" class="flex items-center justify-between">
                  <span class="text-sm text-yellow-700">Количество:</span>
                  <div class="text-right">
                    <div class="text-xs line-through text-gray-500">{{ originalData.amount }} единиц</div>
                    <div class="font-medium text-yellow-900">{{ form.amount }} единиц</div>
                  </div>
                </div>
              </div>
            </div>
      ...
      <div v-if="!isLoading && !error" class="mt-4 bg-gray-50 border border-gray-200 rounded-lg p-4">
...
const hasChanges = computed(() => {
  return form.name !== originalData.name ||
      form.type !== originalData.type ||
      parseInt(form.amount) !== originalData.amount
})
...
const loadResourceData = async () => {
  try {
    ...
    if (data.success && data.data) {
      ...
      originalData.name = data.data.name || ''
      originalData.type = data.data.type || ''
      originalData.amount = data.data.amount || 0
    } else {
      throw new Error(data.message || 'Данные материала не найдены')
    }
...
const handleCancel = () => {
  if (hasChanges.value) {
    if (confirm('У вас есть несохраненные изменения. Вы уверены, что хотите уйти?')) {
      router.push(`/things/resources/view/${resourceId}`)
    }
  } else {
    router.push(`/things/resources/view/${resourceId}`)
  }
...
const handleSubmit = async () => {
  ...
  try {
    ...
    const response = await axios.put(`${BACKEND_URL}/api/resources/${resourceId}`, updateData)
    ...
    if (err.response) {
      ...
      if (err.response.status === 422) {
        ...
        submitError.value = 'Материал не найден'
      } else {
