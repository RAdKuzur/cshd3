        ...
        <div class="flex justify-between items-center">
          <div>
            <div class="flex items-center gap-3 mb-2">
              ...
              <h1 class="text-3xl font-bold text-gray-900">История перемещений</h1>
            </div>
            <p class="text-gray-600">Управление историей пополнений и расходов расходных материалов</p>
          </div>
          <button
      ...
      <div v-else class="bg-white rounded-2xl shadow-lg border border-gray-200 overflow-hidden">
        <div class="overflow-x-auto">
          <table class="w-full">
            ...
            <tr
                v-for="history in paginatedItems"
                :key="history.id"
...
<!--                    <span class="text-sm font-bold text-indigo-700">#{{ history.id }}</span>-->
<!--                  </div>-->
<!--                  <div class="ml-4">-->
...
<!--              </td>-->
              ...
              <td class="px-6 py-4">
                <div class="flex items-center">
                  <div class="flex-shrink-0 h-8 w-8 rounded-lg flex items-center justify-center mr-3 shadow-sm"
                       :class="history.amount > 0 ? 'bg-gradient-to-r from-green-100 to-emerald-100' : 'bg-gradient-to-r from-red-100 to-pink-100'">
                    <svg v-if="history.amount > 0" class="h-4 w-4 text-green-600" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                      <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4" />
                    </svg>
                  ...
                  <div>
                    <div class="text-sm font-semibold" :class="history.amount > 0 ? 'text-green-600' : 'text-red-600'">
                      {{ history.amount > 0 ? '+' : '' }}{{ history.amount }}
                    </div>
                    <div class="text-xs text-gray-500">
                      {{ history.amount > 0 ? 'Пополнение' : 'Расход' }}
                    </div>
                  </div>
                </div>
              </td>

              <!-- Текущий остаток (вычисляем) -->
              <td class="px-6 py-4">
                <div class="flex items-center">
                  ...
                  <div>
                    <div class="text-sm font-semibold text-gray-900">
                      {{ getResourceAmount(history.resource_id) }}
                    </div>
                    <div class="text-xs text-gray-500">
                      текущий остаток
                    </div>
        ...
        <!-- Пустое состояние -->
        <div v-if="!isLoading && filteredHistory.length === 0" class="text-center py-12">
          <svg class="mx-auto h-12 w-12 text-gray-400" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" />
          </svg>
          <h3 class="mt-2 text-sm font-medium text-gray-900">Записи не найдены</h3>
          <p class="mt-1 text-sm text-gray-500">
            {{ searchQuery || typeFilter !== 'all' ? 'Попробуйте изменить параметры поиска' : 'Добавьте первую запись' }}
          </p>
        </div>

        <!-- Пагинация -->
        <div v-if="!isLoading && filteredHistory.length > 0" class="px-6 py-4 border-t border-gray-200 bg-gray-50">
          <div class="flex items-center justify-between flex-wrap gap-4">
            <div class="text-sm text-gray-700">
              Показано с {{ startIndex }} по {{ endIndex }} из {{ filteredHistory.length }} записей
            </div>
            <div class="flex items-center space-x-2">
              <button
                  @click="prevPage"
          ...
          <div class="p-6">
            <div class="flex justify-between items-center mb-6">
              <h3 class="text-xl font-bold text-gray-900">
                {{ editingHistory ? 'Редактировать запись' : 'Добавить запись' }}
              </h3>
              <button
                  @click="closeModal"
            ...
            <form @submit.prevent="saveHistory">
              <div class="mb-6">
                ...
                <div v-if="resourcesLoading" class="mt-2 text-sm text-gray-500">
                  <svg class="animate-spin inline h-4 w-4 mr-2 text-blue-600" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
                    <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle>
                    <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
                  </svg>
              ...
              <div class="mb-6">
                ...
                <div class="flex gap-3">
                  <button
                      type="button"
                      @click="form.amount = Math.abs(form.amount)"
                      :class="[
                        'flex-1 px-4 py-3 rounded-lg font-medium transition-all border',
                        form.amount >= 0
                          ? 'bg-gradient-to-r from-green-600 to-emerald-600 text-white border-transparent shadow-sm'
                  ...
                  <button
                      ...
                      :class="[
                        'flex-1 px-4 py-3 rounded-lg font-medium transition-all border',
                        form.amount < 0
                          ? 'bg-gradient-to-r from-red-600 to-pink-600 text-white border-transparent shadow-sm'
                          : 'bg-white text-gray-700 border-gray-300 hover:bg-gray-50'
                      ]"
                  >
                    <span class="flex items-center justify-center gap-2">
              ...
              <div class="mb-6">
                ...
                <input
                    ...
                    :placeholder="form.amount >= 0 ? 'Введите количество для пополнения' : 'Введите количество для расхода'"
                    :disabled="isSaving"
                />
                <p v-if="formErrors.amount" class="mt-2 text-sm text-red-600">{{ formErrors.amount }}</p>
              </div>
              ...
              <div class="flex justify-end gap-3">
                ...
                >
                  ...
                  {{ isSaving ? 'Сохранение...' : (editingHistory ? 'Сохранить' : 'Добавить') }}
                </button>
              </div>
            </form>
          </div>
          ...
          <div class="p-6">
            <div class="text-center">
              ...
              <p class="text-gray-600 mb-6">
                ...
                <span class="font-semibold text-gray-900">{{ getOperationType(historyToDelete?.amount) }}</span>
                материала
                <span class="font-semibold text-gray-900">"{{ getResourceName(historyToDelete?.resource_id) }}"</span>?
...
const headers = ref([
  ...
  { key: 'amount', label: 'Операция' },
  { key: 'current_amount', label: 'Остаток' }
])
...
// Получение названия ресурса по ID
...
// Получение типа операции для отображения
const getOperationType = (amount) => {
  return amount > 0 ? 'пополнении' : 'расходе'
}
...
const loadData = async () => {
  try {
    ...
    const response = await axios.get(`${BACKEND_URL}/api/history/resources`)
...
const filteredHistory = computed(() => {
  ...
  if (searchQuery.value) {
    ...
    filtered = filtered.filter(item =>
        getResourceName(item.resource_id).toLowerCase().includes(query)
    )
  }
  ...
  filtered = [...filtered].sort((a, b) => {
    ...
    if (sortKey.value === 'resource_id') {
      aVal = getResourceName(a.resource_id)
      bVal = getResourceName(b.resource_id)
    } else if (sortKey.value === 'current_amount') {
      aVal = getResourceAmount(a.resource_id)
      bVal = getResourceAmount(b.resource_id)
    } else {
      aVal = aVal || 0
      bVal = bVal || 0
    }
...
const endIndex = computed(() => Math.min(currentPage.value * itemsPerPage.value, filteredHistory.value.length))
...
const openEditModal = async (historyItem) => {
  editingHistory.value = historyItem
  form.value = {
    resource_id: historyItem.resource_id,
...
const openDeleteModal = (historyItem) => {
  historyToDelete.value = historyItem
...
const saveHistory = async () => {
  try {
    ...
    const url = editingHistory.value
        ? `${BACKEND_URL}/api/history/resources/${editingHistory.value.id}`
        : `${BACKEND_URL}/api/history/resources`
    ...
    if (err.response?.data?.errors) {
      ...
      formErrors.value = {
        resource_id: err.response?.data?.message || err.message || 'Ошибка сохранения'
      }
      showNotification(
...
const confirmDelete = async () => {
  ...
  try {
    ...
    const response = await axios.delete(`${BACKEND_URL}/api/history/resources/${historyToDelete.value.id}`)
