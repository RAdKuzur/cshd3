  ...
  <div class="p-6 bg-gradient-to-br from-gray-50 to-blue-50 min-h-screen">
    <div class="max-w-7xl mx-auto">
      ...
      <div class="mb-8">
        <div class="flex justify-between items-center">
          <div>
            ...
            <p class="text-gray-600">Управление отделами компании</p>
          </div>
          <button
              @click="openCreateModal"
              class="bg-gradient-to-r from-green-600 to-emerald-600 hover:from-green-700 hover:to-emerald-700 text-white px-6 py-3 rounded-lg font-semibold shadow-sm transition-all duration-200 flex items-center gap-2 hover:shadow-md"
          >
            <svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        ...
        <div class="mt-6">
          <div class="relative max-w-md">
            <input
                ...
                class="w-full pl-10 pr-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-green-500 shadow-sm bg-white"
            >
      ...
      <div v-else class="bg-white rounded-2xl shadow-lg border border-gray-200 overflow-hidden">
          ...
          <table class="w-full">
            ...
            <tr>
              <th
                  ...
                  :key="header.key"
                  class="px-6 py-4 text-left text-sm font-semibold text-white uppercase tracking-wider cursor-pointer hover:bg-emerald-700 transition-colors"
            ...
            <tr
                v-for="branch in paginatedItems"
                :key="branch.id"
            ...
            >
              ...
              <td class="px-6 py-4 whitespace-nowrap">
                <div class="flex items-center">
                  <div class="flex-shrink-0 h-10 w-10 bg-gradient-to-br from-green-100 to-emerald-100 rounded-lg flex items-center justify-center shadow-sm">
                    <span class="text-sm font-bold text-emerald-700">#{{ branch.id }}</span>
                  </div>
                  <div class="ml-4">
              ...
              <td class="px-6 py-4">
                <div class="flex items-center">
                  ...
                  <div>
                    <div class="text-lg font-semibold text-gray-900 group-hover:text-emerald-600 transition-colors">
                      {{ branch.name }}
                    </div>
        ...
        <!-- Пустое состояние -->
        <div v-if="!isLoading && filteredBranches.length === 0" class="text-center py-12">
          <svg class="mx-auto h-12 w-12 text-gray-400" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-5 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4" />
          </svg>
          <h3 class="mt-2 text-sm font-medium text-gray-900">Отделы не найдены</h3>
          <p class="mt-1 text-sm text-gray-500">
            {{ searchQuery ? 'Попробуйте изменить параметры поиска' : 'Добавьте первый отдел' }}
          </p>
        </div>

        <!-- Пагинация -->
        <div v-if="!isLoading && filteredBranches.length > 0" class="px-6 py-4 border-t border-gray-200 bg-gray-50">
          <div class="flex items-center justify-between flex-wrap gap-4">
            <div class="text-sm text-gray-700">
              Показано с {{ startIndex }} по {{ endIndex }} из {{ filteredBranches.length }} записей
            </div>
            <div class="flex items-center space-x-2">
              <button
                  @click="prevPage"
            ...
            <select
                ...
                class="px-3 py-1 border border-gray-300 rounded-md text-sm focus:ring-2 focus:ring-green-500 focus:border-green-500 bg-white"
            >
          ...
          <div class="p-6">
            <div class="flex justify-between items-center mb-6">
              <h3 class="text-xl font-bold text-gray-900">
                {{ editingBranch ? 'Редактировать отдел' : 'Создать отдел' }}
              </h3>
              <button
                  @click="closeModal"
            ...
            <form @submit.prevent="saveBranch">
              ...
              <div class="flex justify-end gap-3">
                ...
                <button
                    type="submit"
                    :disabled="isSaving"
                    class="px-6 py-2.5 bg-gradient-to-r from-green-600 to-emerald-600 text-white font-medium rounded-lg hover:from-green-700 hover:to-emerald-700 disabled:opacity-50 disabled:cursor-not-allowed transition-all shadow-sm hover:shadow flex items-center gap-2"
                >
                  <svg v-if="isSaving" class="animate-spin h-4 w-4 text-white" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
                    <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle>
                    <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
                  </svg>
                  {{ isSaving ? 'Сохранение...' : (editingBranch ? 'Сохранить' : 'Создать') }}
                </button>
              </div>
            </form>
          </div>
        ...
        <div class="relative bg-white rounded-2xl shadow-2xl w-full max-w-md transform transition-all duration-300 scale-100">
          <div class="p-6">
            <div class="text-center">
              ...
              <h3 class="text-lg font-bold text-gray-900 mb-2">Удалить отдел</h3>
              <p class="text-gray-600 mb-6">
                Вы уверены, что хотите удалить отдел
                <span class="font-semibold text-gray-900">"{{ branchToDelete?.name }}"</span>?
...
const headers = ref([
  { key: 'id', label: 'ID' },
  { key: 'name', label: 'Название отдела' }
])
...
const loadData = async () => {
  try {
    ...
    const response = await axios.get(BACKEND_URL + '/api/admin/branches')

    if (response.data.success) {
      branches.value = response.data.data || []
    } else {
      throw new Error(response.data.message || 'Не удалось загрузить список отделов')
    }
  } catch (err) {
...
const endIndex = computed(() => Math.min(currentPage.value * itemsPerPage.value, filteredBranches.value.length))
...
const saveBranch = async () => {
  try {
    ...
    const url = editingBranch.value
        ? `${BACKEND_URL}/api/admin/branches/${editingBranch.value.id}`
        : `${BACKEND_URL}/api/admin/branches`
    ...
    console.error('Ошибка при сохранении отдела:', err)
...
const confirmDelete = async () => {
  ...
  try {
    ...
    const response = await axios.delete(`${BACKEND_URL}/api/admin/branches/${branchToDelete.value.id}`)

    if (response.data.success) {
      // Показываем уведомление об успехе
      showNotification('Отдел успешно удален')
