  ...
  <div class="p-6 bg-gradient-to-br from-gray-50 to-blue-50 min-h-screen">
    <div class="max-w-7xl mx-auto">
        ...
        <div class="flex justify-between items-center">
          <div>
            <div class="flex items-center gap-3 mb-2">
              ...
              <h1 class="text-3xl font-bold text-gray-900">Модели устройств</h1>
            </div>
            <p class="text-gray-600">Управление моделями устройств и их производителями</p>
          </div>
          <button
      ...
      <div v-else class="bg-white rounded-2xl shadow-lg border border-gray-200 overflow-hidden">
            ...
            >
              ...
              <td class="px-6 py-4 whitespace-nowrap">
                <div class="flex items-center">
                  <div class="flex-shrink-0 h-10 w-10 bg-gradient-to-br from-blue-100 to-indigo-100 rounded-lg flex items-center justify-center shadow-sm">
                    <span class="text-sm font-bold text-indigo-700">#{{ model.id }}</span>
                  </div>
                  <div class="ml-4">
              ...
              <td class="px-6 py-4">
                <div class="flex items-center">
                  ...
                  <div>
                    <div class="text-sm font-semibold text-gray-900">
                      {{ getCompanyName(model.company_id) || 'Не указан' }}
                    </div>
                    <div class="text-xs text-gray-500">
                      ID: {{ model.company_id }}
                    </div>
        ...
        <!-- Пустое состояние -->
        <div v-if="!isLoading && filteredModels.length === 0" class="text-center py-12">
          <svg class="mx-auto h-12 w-12 text-gray-400" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 3v2m6-2v2M9 19v2m6-2v2M5 9H3m2 6H3m18-6h-2m2 6h-2M7 19h10a2 2 0 002-2V7a2 2 0 00-2-2H7a2 2 0 00-2 2v10a2 2 0 002 2zM9 9h6v6H9V9z" />
          </svg>
          <h3 class="mt-2 text-sm font-medium text-gray-900">Модели не найдены</h3>
          <p class="mt-1 text-sm text-gray-500">
            {{ searchQuery ? 'Попробуйте изменить параметры поиска' : 'Добавьте первую модель' }}
          </p>
        </div>

        <!-- Пагинация -->
        <div v-if="!isLoading && filteredModels.length > 0" class="px-6 py-4 border-t border-gray-200 bg-gray-50">
          <div class="flex items-center justify-between flex-wrap gap-4">
            <div class="text-sm text-gray-700">
              Показано с {{ startIndex }} по {{ endIndex }} из {{ filteredModels.length }} записей
            </div>
            <div class="flex items-center space-x-2">
              <button
                  @click="prevPage"
          ...
          <div class="p-6">
            ...
            <form @submit.prevent="saveModel">
              ...
              <div class="mb-6">
                ...
                <p v-if="formErrors.company_id" class="mt-2 text-sm text-red-600">{{ formErrors.company_id }}</p>
                <div v-if="companiesLoading" class="mt-2 text-sm text-gray-500">
              ...
              <div class="flex justify-end gap-3">
                ...
                >
                  ...
                  {{ isSaving ? 'Сохранение...' : (editingModel ? 'Сохранить' : 'Создать') }}
                </button>
              </div>
            </form>
          </div>
        ...
        <div class="relative bg-white rounded-2xl shadow-2xl w-full max-w-md transform transition-all duration-300 scale-100">
          <div class="p-6">
            <div class="text-center">
              ...
              <h3 class="text-lg font-bold text-gray-900 mb-2">Удалить модель</h3>
              <p class="text-gray-600 mb-6">
                Вы уверены, что хотите удалить модель
                <span class="font-semibold text-gray-900">"{{ modelToDelete?.name }}"</span>?
...
const headers = ref([
  ...
  { key: 'company_id', label: 'Производитель' }
])
...
const loadData = async () => {
  try {
    ...
    if (response.data.success) {
      ...
      throw new Error(response.data.message || 'Не удалось загрузить список моделей')
    }
  } catch (err) {
...
const filteredModels = computed(() => {
  ...
  if (searchQuery.value) {
    ...
    filtered = filtered.filter(model =>
        ...
        getCompanyName(model.company_id)?.toLowerCase().includes(query)
    )
  }
  ...
  filtered = [...filtered].sort((a, b) => {
    ...
    // Специальная обработка для company_id - сортировка по названию компании
    if (sortKey.value === 'company_id') {
...
const endIndex = computed(() => Math.min(currentPage.value * itemsPerPage.value, filteredModels.value.length))
...
const openCreateModal = async () => {
  ...
  // Обновляем список компаний перед открытием модального окна
  await loadCompanies()
...
const saveModel = async () => {
  try {
    ...
    const url = editingModel.value
        ? `${BACKEND_URL}/api/models/${editingModel.value.id}`
...
const confirmDelete = async () => {
  ...
  try {
    ...
    const response = await axios.delete(`${BACKEND_URL}/api/models/${modelToDelete.value.id}`)
