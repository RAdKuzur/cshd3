    ...
    <div class="max-w-6xl mx-auto">
      ...
      <div class="mb-8">
        <div class="flex items-center justify-between mb-6">
          <div>
            <h1 class="text-2xl md:text-3xl font-bold text-gray-900">Сетевой аудит</h1>
            <p class="text-gray-600 mt-2">Мониторинг и анализ сетевой инфраструктуры</p>
          </div>
          <router-link
              to="/reports"
      ...
      <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
          ...
          <div class="flex items-start gap-3">
            ...
            <div>
              <p class="text-blue-800 text-sm">
                Формирование отчета по сетевой инфраструктуре. Отчет включает информацию
                о сетевом оборудовании, IP-адресации и телефонии.
              </p>
            </div>
          </div>
        </div>
        ...
        <div class="max-w-md mx-auto">
          <div class="bg-gradient-to-r from-blue-600 to-indigo-700 text-white rounded-xl p-6 shadow-lg">
            ...
            <div class="relative z-10">
              ...
              <div class="flex items-center gap-4 mb-6">
                ...
                <div>
                  <h2 class="text-2xl font-bold">Сетевой аудит</h2>
                  <p class="text-blue-100/80 text-sm mt-1">Анализ сетевой инфраструктуры</p>
                </div>
              </div>
              ...
              <div class="mb-6">
                <div class="grid grid-cols-2 gap-4 mb-4">
                  <div class="bg-white/10 p-3 rounded-lg">
                    <div class="text-xs text-blue-100/70 mb-1">Формат</div>
                    <div class="font-medium">Excel</div>
                  </div>
                  <div class="bg-white/10 p-3 rounded-lg">
                    <div class="text-xs text-blue-100/70 mb-1">Обновление</div>
                    <div class="font-medium">В реальном времени</div>
                  </div>
                </div>

                <ul class="space-y-2 text-blue-100/90 text-sm">
              ...
              <!-- Кнопка выгрузки -->
              <button
                  @click="generateNetworkAuditReport"
                  :disabled="loading"
                  class="w-full bg-white text-blue-600 hover:bg-gray-100 font-semibold py-3 px-4 rounded-lg transition-all duration-300
              ...
              >
                ...
                <template v-else>
                  ...
                  <span>Выгрузить отчёт по сетевому аудиту</span>
                </template>
              </button>

              <!-- Информация под кнопкой -->
              <div class="mt-4 text-center text-blue-100/70 text-xs">
                <p>Отчет будет сформирован на основе текущих данных о сетевой инфраструктуре</p>
              </div>
            </div>
          </div>
        </div>
...
// Формирование отчета по сетевому аудиту
const generateNetworkAuditReport = async () => {
  try {
    loading.value = true

    const response = await axios.get('/api/reports/network-audit', {
      responseType: 'blob'
    })
    ...
    let filename = 'network_audit_report.xlsx'
    ...
    showNotification('Отчет по сетевому аудиту успешно сформирован', 'success')
